    ...
    _.times(NB_LOOP, function() {
      ...
      _.times(problem.nsteps, function(istep) {
        _(loons)
          ...
          .value();
        _.each(loons, function addScoreToLoon(loon) {
          if (loon.altitude !== 0) {
            var nbLoonLimit = 0;
            _.each(xLimit, function(distX) {
              _.each(yLimit, function(distY) {
                ...
                if (dx < 0) dx += problem.nx;
                var dy = loon.y + distY;
                if (_.find(loons, {x: dx, y: dy})) {
                  nbLoonLimit += 1;
                }
              });
            });
            //console.log("nbLoonLimit>", nbLoonLimit, " for loon:", {x: loon.x, y: loon.y});
            if (nbLoonLimit === 1) {
              loon.score = (loon.score || 0) + (problem.heatMap[loon.y][loon.x]);
            }
          }
